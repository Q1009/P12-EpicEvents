...
from textual.widgets import Header, Footer, DataTable, Button, Input, Label, Select
...
class CollaboratorScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes='collaborator-main-container'):
            ...
            with Container(classes='collaborator-buttons-container'):
                yield Button('Create Collaborator', id='create-collaborator', variant='primary')
                yield Button('Update Collaborator', id='update-collaborator', variant='warning')
                yield Button('Delete Collaborator', id='delete-collaborator', variant='error')
            with Container(classes='customers-events-buttons-container'):
                yield Button('Consult Customer', id='consult-customer', variant='primary')
    ...
    def load_collaborators(self, table: DataTable) -> None:
        ...
        for collaborator in self.collaborators:
            # Department
            department = (
                collaborator.department.name.value
            )
            # Number of customers
    ...
    def load_collaborator_customers(self, table: DataTable, collaborator: Collaborator) -> None:
        table.clear()
    ...
    def load_collaborator_events(self, table: DataTable, collaborator: Collaborator) -> None:
        table.clear()
    ...
    def watch_selected_collaborator_id(self, new_id: int | None) -> None:
        ...
        """
        collaborator_customers_table = self.query_one('#collaborator-customers-table', DataTable)
        collaborator_events_table = self.query_one('#collaborator-events-table', DataTable)
        ...
        selected_collaborator = next(
            (c for c in self.collaborators if c.id == new_id),
            None
        )

        if selected_collaborator:
            self.load_collaborator_customers(collaborator_customers_table, selected_collaborator)
            self.load_collaborator_events(collaborator_events_table, selected_collaborator)
    ...
    def on_customer_row_highlighted(self, event: DataTable.RowHighlighted) -> None:
        ...
        if self.selected_collaborator_id:
            selected_collaborator = next(
                (c for c in self.collaborators if c.id == self.selected_collaborator_id),
                None
            )
            if selected_collaborator and 0 <= row_index < len(selected_collaborator.customers):
    ...
    def on_event_row_highlighted(self, event: DataTable.RowHighlighted) -> None:
        ...
        if self.selected_collaborator_id:
            ...
            if selected_collaborator and 0 <= row_index < len(selected_collaborator.events):
...
class CreateCollaboratorScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes='create-collaborator-main-container'):
            with Container(id='collaborator-data', classes='collaborator-data-input-container'):
                yield Label("Collaborator Last Name:", classes="form-label")
                yield Input(placeholder="Doe", id="collaborator_last_name", type='text', classes="form-input")
                yield Label("Collaborator First Name:", classes="form-label")
                yield Input(placeholder="John", id="collaborator_first_name", type='text', classes="form-input")
                yield Label("Password:", classes="form-label")
                yield Input(placeholder="my-secure-password", id="password", password=False, classes="form-input")
            with Container(id='collaborator-department', classes='collaborator-department-input-container'):
                department_options = [
            ...
            with Container(classes='create-collaborator-buttons-container'):
                yield Button("Create", id="create", variant="primary", classes='create-collaborator-button')
                yield Button("Cancel", id="cancel", variant="default", classes='create-collaborator-button')
        yield Footer(show_command_palette=False)

    def on_mount(self) -> None:
        collaborator_data_container = self.query_one('#collaborator-data', Container)
        collaborator_department_container = self.query_one('#collaborator-department', Container)
    ...
    def _collect_form_data(self) -> dict:
        ...
        self.customer_data = {
            "collaborator_last_name": self.query_one("#collaborator_last_name", Input).value,
            "collaborator_first_name": self.query_one("#collaborator_first_name", Input).value,
            "password": self.query_one("#password", Input).value,
            "department": self.query_one("#collaborator-department-select", Select).value,
        }